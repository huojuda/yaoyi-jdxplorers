...
class YaoyiHandler(SimpleHTTPRequestHandler):
    ...
    def _handle_auth(self):
        try:
            ...
            if action == "register":
                ...
                self._send_json(400, {"error": "未知操作，应为 register/login/resetpw/delete/diag/me/logout"})
    ...
    def _auth_resetpw(self, data):
        ...
        与 edge-functions/api/auth.js 的 handleResetPw 保持同构。
        国内短信签名对个人主体不开放，改为持手机号直接重置，
        依赖失败锁定 + 冷却做基础防护。生产环境须恢复二次验证。
        """
        phone = str(data.get("phone", "")).strip()
        password = str(data.get("password", ""))
        ...
        if password.isdigit() or password.isalpha():
            self._send_json(400, {"error": "密码需同时包含字母和数字，至少6位"})
            return

        fail_key = "resetpw:fail:" + phone
        fails = int(redis_cmd("GET", fail_key).get("result") or "0")
        if fails >= 5:
            ttl = int(redis_cmd("TTL", fail_key).get("result") or 600)
            self._send_json(429, {"error": f"重置过于频繁，请{max(ttl, 1)}秒后重试"})
            return

        cd = redis_cmd("GET", "resetpw:cd:" + phone).get("result")
        if cd:
            self._send_json(429, {"error": f"操作过于频繁，请{cd}秒后再试", "retryAfter": int(cd)})
            return

        user_id = redis_cmd("GET", "user:phone:" + phone).get("result")
        if not user_id:
            redis_cmd("INCR", fail_key)
            redis_cmd("EXPIRE", fail_key, "600")
        ...
        # 清理冷却、失败计数与登录锁定
        redis_cmd("DEL",
                  "resetpw:cd:" + phone, fail_key,
                  "resetcode:" + phone, "resetcode:cd:" + phone,
                  "resetcode:fail:" + phone, "resetcode:n:" + phone,
                  "login:fail:" + phone)
        redis_cmd("SET", "resetpw:cd:" + phone, "60", "EX", "60")
